...
a = np.min(inputImage[H1:H2+1,W1:W2+1,1])
b = np.max(inputImage[H1:H2+1,W1:W2+1,1])
A, B = 0, 255
...
for i in range(H1, H2+1) :
    for j in range(W1, W2+1) :
        ...
        tmp1[i, j] = [x, (y-a)/(b-a)*B+A, z]
cv2.imshow("xyz_scaling", cv2.cvtColor(tmp1,cv2.COLOR_XYZ2BGR))
